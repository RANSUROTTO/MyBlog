...
### Redis Caching
  1.博客线上版本因为仅仅一个服务器的原因，而且使用了Windows Server 2012,所以仅考虑使用了Redis-Windows-3.2.100.使用StackExchange.Redis作为c#的操作类库。<br/>
  2.在命名空间 Blog.Presentation.Framework 下找到 DependencyRegistrar.cs , 这是一个实现 IDependencyRegistrar 优先级最高的依赖注册类.实现 Register 方法.在该方法中完成 RedisCacheManager 为 ICacheManager 的依赖注册。
  ``` csharp
